...
内存隔离是系统安全的基础。在大多数操作系统中，用户态程序不能直接访问内核空间的内存。在 Linux 32 位操作系统中内核空间的内存地址范围为 `0xC0000000` 到 `0xFFFFFFFF`。这
...
C 语言当中并没有像 Python, C++, Java 等语言的 try-catch 语句，但是我们不希望我们的程序因为访问非法内存地址而崩溃，所以我们使用了信号处理机制来捕获异常。`sigsetjmp` 函数类似于 `setjmp` 函数，第一次遇到这个函数时它会保存当前的执行环境（包括栈指针、程序计数器等），并返回 0。引发异常时，catch_segv 函数就会被调用，从而调用 `siglongjmp`，程序会跳转回之前保存的环境，并且 `sigsetjmp` 会返回一个非零值。这样，我们就可以在 `meltdown()` 函数中访问非法内存地址时引发异常，并通过信号处理程序跳转回安全的执行点，而不会导致程序崩溃，让程序能够继续执行下去，实现再次加载待探测的数组。
***
### 优化手段
