...
!!! Definition "二项树"
	...
	显然能够得到，**二项树并非二叉树**；更进一步的，$k$ 阶二项树 $B_k$​ 的根有 $k$ 个 child，$2^k$ 个节点；再进一步的，$B_k$​ 的第 $d$ 层一共有 $C_k^d$ 个节点，有趣的是，所有层的节点加起来刚好符合二项式定理，此条定理可以由数学归纳法很容易地证明，不再赘述。
...
插入结点可以看作合并一个只有一个结点的二项树，所以我们可以直接复用合并过程。
***
#### Find-Min
...
因此我们联想到，将队首出队问题转化为合并二项队列的问题——假设 $T$ 的队首是 $B_k$ 的根，则队首出队可以转化为求解 $\text{merge}(T−B_k,B_k.\text{root}.\text{children})$ 的问题。
...
先说结论，对于一个含有 $N$ 个元素的二项队列，建立这样的一个队列需要 $N$ 次插入，时间复杂度为 $O(N)$。

!!! Proof

	=== "Aggregate"
	
		![](../../../assets/Pasted image 20241015110555.png)
		
		![](../../../assets/Pasted image 20241015112403.png)
	
	=== "Potential"
	
		我们设 $c_i$ 为第 $i$ 次插入需要的代价，定义 $\Phi_i$ 为第 $i$ 次插入后二项队列中树的棵数（显然 $\Phi_0=0$）
		
		我们从特征比特向量的角度来看这个，假设当前二项队列的特征比特向量为 $...B_{k+1}B_k...B_2B_1B_0(k\geq 0)$，其中 $B_k=B_{k-1}=...=B_2=B_1=B_0=1,B_{k+1}=0(k\geq 0)$，再设 $x$ 为大于 k+1 位的所有位 1 的个数（即二项队列所有树 size 大于 $2^{k+1}$ 的树的棵数）
		
		这时我们有 $c_i+(\Phi_i-\Phi_{i-1})=k+2+(x+1)-(x+k+1)=2(i=1,2,...,N)$
		
		累加所有式子，我们有 $\sum\limits_{i=1}^n c_i+\Phi_n-\Phi_0=2N$
		
		因此 $\sum\limits_{i=1}^n c_i=2N-\Phi_n\leq 2N=O(N)$
