...
一个离散无记忆信源，它的输出字符表为 $\mathcal{A} = \{a_1,a_2,\cdots,a_K\}$，相应的概率分布为 $\{p_1,p_2,\cdots,p_K\}$，满足 $\sum\limits_{i=1}^Kp_i=1$。如果信源输出长度为 $L$ 的消息序列为 $\pmb{u}^L=(u_1,u_2,\cdots,u_L)$，则信源可能输出 $K^L$ 种长度为 $L$ 的不同序列
